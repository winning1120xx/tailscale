...
func (b *LocalBackend) NetworkLockGenerateRecoveryAUM(removeKeys []tkatype.KeyID) (*tka.AUM, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	if b.tka == nil {
		return nil, errNetworkLockNotActive
	...
	if !b.tka.authority.KeyTrusted(nlPriv.KeyID()) {
		return nil, errors.New("this node is not trusted by network lock")
	}

	aum, err := b.tka.authority.MakeRetroactiveRevocation(b.tka.storage, removeKeys)
	if err != nil {
		return nil, err
	}
