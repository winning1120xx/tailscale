...
func TestFindParentForRewrite(t *testing.T) {
	...
	// k1 was trusted at genesis, so theres no better rewrite parent
	// than the genesis.
	k1ID, _ := k1.ID()
	k1P, err := a.findParentForRewrite(c.Chonk(), []tkatype.KeyID{k1ID})
	if err != nil {
	...
	k3ID, _ := k3.ID()
	k3P, err := a.findParentForRewrite(c.Chonk(), []tkatype.KeyID{k3ID})
	if err != nil {
	...
	k2P, err := a.findParentForRewrite(c.Chonk(), []tkatype.KeyID{k2ID})
	if err != nil {
	...
	doubleP, err := a.findParentForRewrite(c.Chonk(), []tkatype.KeyID{k2ID, k3ID})
	if err != nil {
...
func TestMakeRetroactiveRevocation(t *testing.T) {
	...
	k2ID, _ := k2.ID()
	k3ID, _ := k3.ID()
	forkingAUM, err := a.MakeRetroactiveRevocation(c.Chonk(), []tkatype.KeyID{k2ID})
	if err != nil {
